...
The snapshot key contains observer/time/calibration/configuration revisions and values, exact UTC instant, paused/mode state, canonical enabled-provider set, P03/body/refraction choices, and each registered provider name/version. An accepted calibration event changes its identity even when it produces equal yaw. This prevents an old observer, room yaw, model, correction profile, provider, or instant from looking current.
