...
export const CONSTELLATION_PRIMARY_COLOR_GROUP: Readonly<Record<Course50ConstellationIdentifier, ConstellationPrimaryColorGroup>> = Object.freeze({
  ...
  HYA: 'spring', ERI: 'autumn', CET: 'autumn', VUL: 'summer', LAC: 'autumn', EQU: 'autumn', SCT: 'summer', SER: 'summer', LUP: 'summer', CRU: 'summer',
});
...
export function resolveConstellationColor(identifier: Course50ConstellationIdentifier, mode: ConstellationColorMode, strength: ConstellationColorStrength, selectedGroupId: ConstellationLearningGroupId | undefined, baseColorId: ConstellationBaseColorId = 'celestial-lavender', highlightColorId: ConstellationHighlightColorId = 'observation-orange'): ResolvedConstellationColor {
  ...
  const focusedGroup = selectedGroup && !['all-expanded', 'added-only', 'all-course-40', 'all-course-50', 'v3a-additions-only', 'clear'].includes(selectedGroup.id) ? selectedGroup : undefined;
  if (mode === 'highlight') {
